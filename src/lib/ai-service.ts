...
export function resolveModernModel(provider: AIProvider, inputModel?: string): string {
  ...
  if (provider === "gemini") {
    ...
    if (/2\.0/i.test(clean)) return "gemini-3.8-flash";
    if (/(1\.[05]|pro-vision)/i.test(clean)) return "gemini-2.5-flash";
  }
...
async function callGeminiAPI(
  ...
  // Candidate order: user's sanitized model, then active Gemini 3.8 and 2.5 models
